...
@interface OpenCouponView ()

@end

@implementation OpenCouponView
...
//        }
        ...
        if (_bgView==nil) {
            ...
            _bgImageView.image=[UIImage imageNamed:@"open_coupon_bg"];
            ...
            //数量
            _countLbl = [[UILabel alloc] initWithFrame:CGRectMake(127, 143, 50, 34)];
            ...
            self.cancelBtn.frame = CGRectMake(37, 217, 92, 28);
            [self.cancelBtn setBackgroundImage:[UIImage imageNamed:@"open_coupon_btn"] forState:UIControlStateNormal];
            ...
            self.confirmBtn.frame = CGRectMake(157, 217, 92, 28);
            [self.confirmBtn setBackgroundImage:[UIImage imageNamed:@"open_coupon_btn"] forState:UIControlStateNormal];
            ...
            [self.confirmBtn setTitle:@"去看看" forState:UIControlStateNormal];
            [self.confirmBtn addTarget:self action:@selector(checkDetailByClick:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)show:(NSString *)content
{
    if (self.parentCtrl&&content) {
        ...
        if ([self.parentCtrl.view.subviews containsObject:self]==NO) {
            ...
            NSString *countStr = [self subStringFromString:content End:str];
            
            if (countStr) {
                countStr = [self subStringFromString:countStr Begin:@"张"];
...
//                CGSize textSize = [str sizeWithFont:font constrainedToSize:CGSizeMake(MAXFLOAT, 30)];
                CGSize textSize = [str boundingRectWithSize:CGSizeMake(MAXFLOAT, 30) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font} context:nil].size;
...
//查看详情
-(void)checkDetailByClick:(id)sender
{
    if (self.snid&&[self.snid trim].length>0) {
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"Open_Coupon_Clicked" object:self.snid];
    }
    [self hide];
}
