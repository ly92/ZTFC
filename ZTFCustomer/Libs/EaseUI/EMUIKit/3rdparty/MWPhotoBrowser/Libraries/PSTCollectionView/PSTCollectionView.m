...
- (id)dequeueReusableCellWithReuseIdentifier:(NSString *)identifier forIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell) {
        ...
        if (_cellNibDict[identifier]) {
            ...
            if (attributes) {
                cell = [[cellClass alloc] initWithFrame:attributes.PSTframe];
            }else {
                cell = [cellClass new];
            }
        }
    ...
    if (view) {
        ...
        if (_supplementaryViewNibDict[kindAndIdentifier]) {
            ...
            if (self.collectionViewLayout) {
                ...
                if (attributes) {
                    view = [[viewClass alloc] initWithFrame:attributes.PSTframe];
                }
            }else {
                view = [viewClass new];
            }
...
- (NSArray *)visibleCells {
    return [[_allVisibleViewsDict allValues] filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
        return [evaluatedObject isKindOfClass:PSTCollectionViewCell.class] && CGRectIntersectsRect(self.bounds, [evaluatedObject PSTframe]);
    }]];
}
...
- (void)scrollToItemAtIndexPath:(NSIndexPath *)indexPath atScrollPosition:(PSTCollectionViewScrollPosition)scrollPosition animated:(BOOL)animated {
    ...
    if (layoutAttributes) {
        CGRect targetRect = [self makeRect:layoutAttributes.PSTframe toScrollPosition:scrollPosition];
...
- (void)updateWithItems:(NSArray *)items {
    ...
    for (PSTCollectionViewUpdateItem *updateItem in items) {
        ...
        if (updateItem.updateAction == PSTCollectionUpdateActionDelete) {
            ...
            CGRect startRect = startAttrs.PSTframe;
            CGRect finalRect = finalAttrs.PSTframe;
