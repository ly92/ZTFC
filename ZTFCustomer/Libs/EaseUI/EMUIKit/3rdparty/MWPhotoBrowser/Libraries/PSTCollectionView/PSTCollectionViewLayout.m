...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p frame:%@ indexPath:%@ elementKind:%@>", NSStringFromClass(self.class), self, NSStringFromCGRect(self.PSTframe), self.indexPath, self.elementKind];
}
...
- (void)setSize:(CGSize)size {
    _size = size;
    _PSTframe = (CGRect){_PSTframe.origin, _size};
}

- (void)setCenter:(CGPoint)center {
    _center = center;
    _PSTframe = (CGRect){{_center.x - _PSTframe.size.width / 2, _center.y - _PSTframe.size.height / 2}, _PSTframe.size};
}

- (void)setFrame:(CGRect)frame {
    _PSTframe = frame;
    _size = _PSTframe.size;
    _center = (CGPoint){CGRectGetMidX(_PSTframe), CGRectGetMidY(_PSTframe)};
}
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    layoutAttributes.frame = self.PSTframe;
